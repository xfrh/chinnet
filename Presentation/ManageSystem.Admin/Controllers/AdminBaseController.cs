...
namespace ManageSystem.Admin.Controllers
{
    ...
    /// 后台所有控制器的基类
    /// </summary>
    public abstract partial class AdminBaseController : BaseController
    {
        ...
        public AdminBaseController()
        {
            ...
            this.SessionManager = EngineContext.Current.Resolve<ISessionManager>();
        ...
        /// <summary>
        /// 如果启用了IP访问控制，这里检查请求的ip地址是否合法
        /// </summary>
        private void CheckRequestIp()
        {
            ...
            //不能访问
            HttpContext.Response.Redirect(Url.Action("Error", "Common", new { message = "您的IP：" + ip + "，被拒绝访问" }));
        }
        ...
        /// <param name="content">日志标题</param>
        /// <param name="detail">详细内容</param>
        protected void InsetActionLog(ActionType type, string title = "",string detail="")
        {
            try
            {
                Userinfo user = this.LoginUserinfo;

                this.ActionLogService.Insert(type, ActionSource.Admin, user.Id, user.Name + "（" + user.LoginId + "）", title, detail);
            }
