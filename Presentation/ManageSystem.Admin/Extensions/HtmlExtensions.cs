...
namespace ManageSystem.Admin.Extensions
{
    public static class HtmlExtensions
    {
            ...
            string buttonsSelector) where T : BaseEntityModel
        {
            ...
            window.AppendLine(string.Format("title: '{0}',", "确认操作吗？"));
        ...
        public static MvcHtmlString NopLabelFor<TModel, TValue>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TValue>> expression, bool displayHint = true)
        {
            ...
            object value;
            bool required = false;
            if (metadata.AdditionalValues.TryGetValue("HtmlDisplayAttribute", out value))
            {
                var htmlDisplayAttribute = value as HtmlDisplayAttribute;
                if (htmlDisplayAttribute != null && displayHint)
                {
                    if (!string.IsNullOrWhiteSpace(htmlDisplayAttribute.HtmlTitle))
                    {
                        hintResource = htmlDisplayAttribute.HtmlTitle;
                        result.Append(helper.Hint(htmlDisplayAttribute.HtmlTitle).ToHtmlString());
                    }

                    required = htmlDisplayAttribute.Required;
                }
            }

            result.Append(helper.LabelFor(expression, new { title = hintResource }));
            if (required)
                result.Append("<span class=\"html-label-required\" >*</span>");
        ...
        /// <returns></returns>
        public static MvcHtmlString ButtonHtml(this HtmlHelper helper, HtmlButtonType type, string actionName,
            string controllerName, object htmlAttributes = null, bool checkRole = true)
        {
            var url = @"/" + controllerName + "/" + actionName;
            if (checkRole && !ManageSystem.Services.Users.UserinfoExtensions.CheckFunction(url)) return MvcHtmlString.Create("");

            switch (type)
            {
                case HtmlButtonType.Add:
                    if (htmlAttributes == null) htmlAttributes = new { @class = "btn btn-purple" };
                    return helper.ActionLink("添 加", actionName, controllerName, null, htmlAttributes);
                case HtmlButtonType.Edit:
                    return helper.ActionLink("编 辑", actionName, controllerName, htmlAttributes);
                case HtmlButtonType.View:
                    return helper.ActionLink("查 看", actionName, controllerName, htmlAttributes);
                case HtmlButtonType.Import:
                    if (htmlAttributes == null) htmlAttributes = new { @class = "btn btn-purple" };
                    return helper.ActionLink("导入数据", actionName, controllerName, htmlAttributes);
                case HtmlButtonType.Delete:
                    return MvcHtmlString.Create("<input type=\"button\" value=\"删 除\" class=\"btn btn-danger\" id=\"btn-delete\" />");
                case HtmlButtonType.Export:
                    break;
            }
        ...
        /// <summary>
        /// 显示kendo UI 的详细链接html
        /// </summary>
        /// <param name="helper"></param>
        /// <param name="isShow">是否显示</param>
        /// <returns></returns>
        public static MvcHtmlString KendoUIView(this HtmlHelper helper, bool isShow, string url)
        {
            if (!isShow) return MvcHtmlString.Create("");

            return MvcHtmlString.Create("<a href='" + url + "/#=Id#'>详细</a>");
        }

        /// <summary>
        /// 显示kendo UI 的编辑链接html
        /// </summary>
        /// <param name="helper"></param>
        /// <param name="isShow">是否显示</param>
        /// <returns></returns>
        public static MvcHtmlString KendoUIEdit(this HtmlHelper helper, bool isShow, string url)
        {
            if (!isShow) return MvcHtmlString.Create("");

            return MvcHtmlString.Create("<a href='" + url + "/#=Id#'>编辑</a>");
        }

        /// <summary>
        /// 显示kendo UI 的编辑链接和查看链接html
        /// </summary>
        /// <param name="helper"></param>
        /// <param name="showEdit">显示编辑链接</param>
        /// <param name="showView">显示查看链接</param>
        /// <param name="editUrl">编辑链接地址</param>
        /// <param name="viewUrl">查看链接地址</param>
        /// <returns></returns>
        public static MvcHtmlString KendoUIEditAndView(this HtmlHelper helper, bool showView = true, string viewUrl = "", bool showEdit = true, string editUrl = "")
        {
            string html = "";

            if (showView)
                html += "<a href='" + viewUrl + "/#=Id#'>详细</a>";

            if (showEdit)
                html += "&nbsp;&nbsp;<a href='" + editUrl + "/#=Id#'>编辑</a>";
        ...
        public static MvcHtmlString MICapplyButton(this HtmlHelper helper)
        {
            string html = "";

            html += "<button class='btn btn-link' type='button' id='MICsqForm' onclick='Agree(#=Smid#)'>同意</button>";
            html += "&nbsp;&nbsp;<button class='btn btn-link' id='MICbhForm' type='button' onclick='Reject(#=Smid#)'>驳回</button>";
