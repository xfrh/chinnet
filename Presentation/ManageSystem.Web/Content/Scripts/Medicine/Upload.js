...
$(document).ready(function () {
    ...
    //    return;
    //}
    if ($('#HiddenHospitalWard').val() === '1') {
        if ($('#HiddenDepartment').val() === '1') {
            layer.confirm('您所在的医院尚未进行科室配置，请点击【确认】进行科室配置，或点击【取消】返回！', { btn: ['确认', '取消'], icon: 0, tite: '提示', closeBtn: 0 }, function () {
                window.location.href = '/HospitalDepartment/HospitalDepartmentList';
            }, function () {
                window.location.href = '/Data/AntibioticDrugFast';
            });
        }
        else {
            layer.alert('您所在的医院尚未进行科室配置，请联系医院相关负责人进行科室配置，或者点击【确认】与我们联系！', { btn: ['确认', '取消'], icon: 0, tite: '提示', closeBtn: 0 }, function () {
                window.location.href = '/Center/Feedback';
            }, function () {
                window.location.href = '/Data/AntibioticDrugFast';
            });
        }
...
function UploadFileCallback(item) {
    ...
    fileArr.push(item.FileName + "^" + item.FilePath);

    var htmlPath = "";
    for (var i = 0; i < fileArr.length; i++) {
        htmlPath += " <a href='" + fileArr[i].split('^')[1] + "' target='_blank' style='color: #428bca;'>" + fileArr[i].split('^')[0] + "</a>  "
    }

    //var html = "数据文件已加载，请点击“开始上传”，<a href='" + item.FilePath + "' target='_blank' style='color: #428bca;'>" + item.FileName + "</a>";
    var html = "数据文件已加载，请点击“开始上传”，" + htmlPath;
...
function Save() {
    ...
    l.start();
     ...
     function_url = projectType == 6 ? "/Medicine/Upload1" : "/Medicine/Upload";
    ...
    $.ajax({
        ...
        data: {
            ...
            FilePathList: fileArr,
            IsSatellite: projectType == 6 ? 1 : 0
        },
        complete: function () {
            l.stop();
        },
        success: function (json) {
            ...
            if (json.Status) {
                ...
                fileArr = new Array();//清空保存的上传路径
                //文件上传成功了，同时已通知接口处理，每隔1秒检查一次处理结果
                intervalResult = setInterval("UpdateMessageTime()", 1000);

            } else {
                if (json.Message == "您所属区域不存在") {
                    layer.open({
                        type: 1,
                        title: '您所在的区域不存在请配置区域',
                        skin: 'layui-layer-rim', //加上边框
                        area: ['520px', '280px'], //宽高
                        content: '<div><ul class="post_main_ul"><li class="post_main_li"><div class= "post_main_l_tit" style="width:157px" > <b class="red">*</b><span>所属区域：</span></div><div style="float:left"><div class="post_main_r" style="position:relative; width:300px;"><select id="Area"></select></div></div></li></ul><input type="button" id="MessageBtn" onclick="updateArea()" class= "btn_Post btn-save" value="确定" style="cursor:pointer; text-align: center;  font-size: 13px; cursor: pointer;width: 120px; height: 35px;line-height: 35px; margin-left: 179px;margin-top: 50px;"/></div> '
                    });
                    GetDistrictList();
                }
...
function CloseMessageWindow() {
    console.log(satelliteCity);
    if (function_url == "/Medicine/Upload1")
        location.href = "/Medicine/Upload1?city=" + satelliteCity;
    else
        location.href = "/Medicine/Upload";
}
