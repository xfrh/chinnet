...
namespace ManageSystem.Web.Controllers
{
    ...
    /// 用户找回密码
    /// </summary>
    public class FindPasswordController : WebBaseController
    {
        ...
        public ContentResult Submit(FindPasswordModel model)
        {
            ...
            //添加日志
            base.InsetActionLog(Core.Domain.Log.ActionType.Edit, "用户找回密码，手机号码：" + model.Phone, entity.SerializeObject());
        ...
        public ContentResult EmailSubmit(FindPasswordModel model)
        {
            ...
            if (entity == null || entity.Id <= 0 || entity.Status != (int)MemberStatus.Normal)
                return this.Content(JsonHelper.GetBaseMessage(false, "帐号或或邮箱不存在"));
            ...
            //添加日志
            base.InsetActionLog(Core.Domain.Log.ActionType.Edit,"用户通过邮箱找回密码，邮箱地址：" + model.Phone, entity.SerializeObject());
