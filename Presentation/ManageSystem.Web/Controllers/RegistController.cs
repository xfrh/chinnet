...
namespace ManageSystem.Web.Controllers
{
    ...
    /// 用户注册
    /// </summary>
    public class RegistController : WebBaseController
    {
        ...
        private bool Islegal(string str)
        {
            ...
            bool result = regExp.IsMatch(str.Trim());
            return result;
        }
        ...
        public ContentResult PostRegist(RegistModel model)
        {
            try
            {
                ...
                if (model.LoginId.Length > 20)
                {
                    return this.Content(JsonHelper.GetBaseMessage(false, "用户名过长！"));
                }

                if (Islegal(model.LoginId))
                {
                    return this.Content(JsonHelper.GetBaseMessage(false, "用户名包含非法字符"));
                }
                ...
                //2022/2/24 根据胡主任文档 修改为 注册使用 登录名或手机号取一个即可
