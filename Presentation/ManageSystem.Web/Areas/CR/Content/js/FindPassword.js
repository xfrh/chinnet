function SendValidateCode() {
    ...
    $.ajax({
        type: "POST",
        url: "/CR/CRFindPassword/SendValidateCode",
        data: { phone: loginId },
        dataType: "json",
...
function SendEmailValidateCode() {
    ...
    $.ajax({
        type: "POST",
        url: "/CR/CRFindPassword/SendEmailValidateCode",
        data: { email: email },
        dataType: "json",
        success: function (result) {
            if (result === null || result === "") {
            ...
            if (result.status === true || result.status === "true") {
                //成功
                var $form_content = $('#Email_PhoneCode.aui-input').closest('div.aui-form-content-item');
                var $yzm = $form_content.find('.yzm');
                $yzm.attr("disabled", "disabled");
            ...
            if (result.status === true || result.status === "true") {
                layer.msg('找回密码成功', {
                    time: 1500 //2秒关闭（如果不配置，默认是3秒）
                        ...
                        $('#myModal').modal('show');
                        $("#regifrom").addClass('hidden')
                        $("#phonepwd").addClass('hidden')
                        $("#emailpwd").addClass('hidden')
                        $("#loginfrom").removeClass('hidden')
                });
            } else {
...
OnEmailSave = function () {
    ...
    $.ajax({
        type: "POST",
        url: "/CR/CRFindPassword/EmailSubmit",
        data: {
            Phone: email,
            ValidateCode: phoneCode,
            Password: password,
