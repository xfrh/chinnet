var mySwiper,
    app = new Vue({
                    ...
                    success: function (res) {
                        if (res.status) {
                            if (res.data.length === 0) {
                                ...
                                _this.pageConfig.pageSize = res.data.length;
        ...
        methods: {
            ...
            download(data) {
                _this = this;
                if (_this.downloadCtrl) {
                    return false;
                }
                $.ajax({
                    url: '/CR/CRDocument/DownloadLog',
                    type: 'POST',
                    data: {
                        documentId: data.Id,
                        token: localStorage.getItem('token')
                    ...
                    timeout: 0,
                    beforeSend: function () {
                        this.downloadCtrl = true;
                    },
                    complete: function () {
                        this.downloadCtrl = false;
                    },
                    error: function () {
                        _this.resFailMessage = '网络请求失败.';
                        $(_this.$refs.failToast).fadeIn(100);
                    ...
                    success: function (res) {
                        if (res.status) {
                            if (res.url == null) {
                                ...
                                location.href = "https://chinets.com" + res.url;
                            }
                        }
                        else {
                            _this.dialogContent = res.message;
