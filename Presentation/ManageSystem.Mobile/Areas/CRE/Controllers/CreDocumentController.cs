...
{
    public class CreDocumentController : MobileBaseController
    {
        #region 业务层声明
        /// <summary>
        /// 资料业务层
        ...
        public CreDocumentController(IDocumentService _documentService, IMemberService _memberService, IEncryptionService _encryptionService, IDocumentDownloadLogService _downloadLogService, ISystemLogService _systemLogService, IActionLogService _actionLogService)
        {
        ...
        public JsonResult List(int page = 1, int pageSize = 15)
        {
            ...
            var pageList = DocumentService.Query(R => R.Owners.Contains("3")).OrderByDescending(r => r.weChattop).ThenBy(r => r.Sort).ThenByDescending(r => r.InsertTime).ToPagedList(page, pageSize);
