...
namespace ManageSystem.Services.Survey
{
    ...
    /// </summary>
    public partial class SurveySubjectOptionService : BaseService<Survey_SubjectOption>, ISurveySubjectOptionService
    {

        public SurveySubjectOptionService(IRepository<Survey_SubjectOption> repository
            ) : base(repository)
        {
        }

        public IPagedList<Survey_SubjectOption> QueryPage(string hospitalName, long hospitalId, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            var list = new PagedList<Survey_SubjectOption>(query, pageIndex, pageSize);
        ...
        /// <returns></returns>
        public IQueryable<Survey_SubjectOption> Query(long memberId)
        {
        ...
        public string Update(Survey_SubjectOption entity, Member member)
        {
            #region 业务处理
        ...
        {
            List<Survey_SubjectOption> list = new List<Survey_SubjectOption>();
            if (ids != null && ids.Any())
            {
                //勾选了数据则导出指定的数据
                list = this.Query(r => ids.Contains(r.Id) && r.Mark > 0).ToList() ?? new List<Survey_SubjectOption>();
            }
            else
            {
                //如果没有选择数据则默认导出全部的数据，根据excel格式，前面显示医院，如果医院没有上传数据的则姓名等数据为空
                var allProject = this.Query(m => m.Mark > 0).ToList() ?? new List<Survey_SubjectOption>(); //查询所有的项目信息
            ...
            list = list.OrderBy(m => m.Id).ToList() ?? new List<Survey_SubjectOption>();
