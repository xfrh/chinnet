...
namespace ManageSystem.Services.Survey
{
    ...
    /// </summary>
    public partial class SurveyRecordService : BaseService<Survey_Record>, ISurveyRecordService
    {

        public SurveyRecordService(IRepository<Survey_Record> repository
            ) : base(repository)
        {
        }

        public IPagedList<Survey_Record> QueryPage(long surveyId, string content29, string content30, string content31, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            {
                query = query.Where(m => m.Content29.Contains(content29));
            }
            if (!string.IsNullOrEmpty(content30))
            {
                query = query.Where(m => m.Content30.Contains(content30));
            }
            if (!string.IsNullOrEmpty(content31))
            {
                query = query.Where(m => m.Content31.Contains(content31));
            }
            ...
            var list = new PagedList<Survey_Record>(query, pageIndex, pageSize);
        ...
        /// <returns></returns>
        public IQueryable<Survey_Record> Query(long memberId)
        {
        ...
        public string Update(Survey_Record entity, Member member)
        {
            #region 业务处理
        ...
        {
            List<Survey_Record> list = new List<Survey_Record>();
            if (ids != null && ids.Any())
            {
                //勾选了数据则导出指定的数据
                list = this.Query(r => ids.Contains(r.Id) && r.Mark > 0).ToList() ?? new List<Survey_Record>();
            }
            else
            {
                //如果没有选择数据则默认导出全部的数据，根据excel格式，前面显示医院，如果医院没有上传数据的则姓名等数据为空
                var allProject = this.Query(m => m.Mark > 0).ToList() ?? new List<Survey_Record>(); //查询所有的项目信息
            ...
            list = list.OrderBy(m => m.Id).ToList() ?? new List<Survey_Record>();
