 ...
 public static class MeetingTypeExtensions
    {
        ...
        public static IList<MeetingType> SortFunctionForTree(this IList<MeetingType> source, long parentId = 0, bool ignoreCategoriesWithoutExistingParent = false)
        {
            if (source == null)
                throw new ArgumentNullException("source");

            var result = new List<MeetingType>();

            foreach (var cat in source.Where(c => c.MeetingTypeId == parentId).ToList())
            {
                result.Add(cat);
        ...
        /// <returns></returns>
        public static string GetFormattedBreadCrumb(this MeetingType model,
           IList<MeetingType> allFunction, string separator = ">>")
        {
        ...
        /// <param name="functionService">Function service</param>
        /// <returns></returns>
        public static IList<MeetingType> GetFunctionBreadCrumb(this MeetingType model,
              IList<MeetingType> allFunction)
        {
            if (model == null)
                ...
                !alreadyProcessedCategoryIds.Contains(model.Id)) //Available
            {
                ...
                model = (from c in allFunction
                         where c.Id == model.MeetingTypeId
        ...
        /// <returns></returns>
        public static string GetFormattedBreadCrumb(this MeetingType model,
            IMeetingTypeService functionService,
           ...
           IMeetingTypeService functionService)
        {
                ...
                !alreadyProcessedCategoryIds.Contains(model.Id)) //prevent circular references
            {
                ...
                model = functionService.QueryEntity(model.MeetingTypeId);
            }
            result.Reverse();
            return result;
        }
