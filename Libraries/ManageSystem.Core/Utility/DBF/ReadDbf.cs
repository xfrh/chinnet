...
namespace ManageSystem.Core.Utility
{
    ...
    public class TDbfTable : IDisposable
    {
        ...
        private DateTime ToDate(byte[] buf)
        {
            ...
            {
                return date.Year > 1900 ? date.Date : NullDateTime;
            }
            else if (DateTime.TryParseExact(dateStr, "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out date))
            {
                return date.Year > 1900 ? date.Date : NullDateTime;
            }
            else if (DateTime.TryParseExact(dateStr, "yyyy.MM.dd", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out date))
            {
                return date.Year > 1900 ? date.Date : NullDateTime;
            }
            else if (DateTime.TryParseExact(dateStr, "yyyy.M.d", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out date))
            {
                return date.Year > 1900 ? date.Date : NullDateTime;
            }
            else
